function findFirstDigit(text: string): string {
...
function replaceWordsWithDigits(input: string): string {
  const words = Object.keys(
    wordToDigitMap,
  ) as unknown as (keyof typeof wordToDigitMap)[];
  let newInput = input;
  for (let word of words) {
    newInput = newInput.replaceAll(
      word,
      word + wordToDigitMap[word].toString() + word,
    );
  }
  return newInput;
}

export function solve01(input: string) {
  ...
  for (let line of lines) {
    ...
    const lastDigit = findFirstDigit(line.split('').reverse().join(''));

    const fullNumber = parseInt(firstDigit + lastDigit);
    sum += fullNumber;
  }
...
export function solve02(input: string) {
  const mappedInput = replaceWordsWithDigits(input);
  const lines = mappedInput.split('\n');
